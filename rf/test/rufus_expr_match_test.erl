    ...
    Expected = [{module, #{line => 2,
                ...
                {func, #{exprs => [{int_lit, #{line => 3,
                         ...
                         return_type => {type, #{line => 3,
                                                 ...
                                                 spec => int}},
                         spec => 'Two'}},
                {func, #{exprs => [{match, #{left => {call, #{args => [],
                                                              line => 4,
                                                              spec => 'Two',
                                                              type => {type, #{line => 3,
                                                                               source => rufus_text,
...
typecheck_and_annotate_function_with_a_match_that_has_a_left_call_operand_with_a_mismatched_right_type_test() ->
    RufusText = "
    module example
    func Two() int { 2 }
    func Random() int {
    ...
    {ok, Tokens} = rufus_tokenize:string(RufusText),
    {ok, Forms} = rufus_parse:parse(Tokens),
    Data = #{globals => #{'Random' => [{func, #{exprs => [{match, #{left => {call, #{args => [],
                                                                                     line => 5,
                                                                                     spec => 'Two'}},
                                                                    line => 5,
                                                                    right => {string_lit, #{line => 5,
...
typecheck_and_annotate_function_with_a_match_that_has_a_left_call_operand_with_a_mismatched_arg_type_test() ->
    RufusText = "
    module example
...
typecheck_and_annotate_function_with_a_match_that_has_unmatched_types_test() ->
    ...
    Data = #{globals => #{'Broken' => [{func, #{exprs => [{match, #{left => {identifier, #{line => 4,
                                                ...
                                                return_type => {type, #{line => 3,
                                                                        ...
                                                                        spec => int}},
                                                spec => 'Broken'}}]},
             left => {identifier, #{line => 6, locals => #{a => {type, #{line => 4,
                                                                         source => inferred,
                                                                         spec => atom}},
                                                           i => {type, #{line => 5,
                                                                         source => inferred,
             ...
             locals => #{a => {type, #{line => 4,
                         ...
                         i => {type, #{line => 5,
                                       source => inferred,
                                       spec => int}}},
             right => {identifier, #{line => 6, locals => #{a => {type, #{line => 4,
                                                                          source => inferred,
                                                                          spec => atom}},
                                                            i => {type, #{line => 5,
                                                                          source => inferred,
