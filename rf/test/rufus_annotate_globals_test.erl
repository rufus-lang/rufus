-module(rufus_annotate_globals_test).
...
%%     ExpectedGlobals = #{},
%%     ?assertEqual({ok, ExpectedGlobals, Forms}, rufus_annotate_globals:forms(Forms)).
...
%%     {ok, Tokens} = rufus_tokenize:string(RufusText),
%%     {ok, Forms} = rufus_parse:parse(Tokens),
%%     {ok, Globals, Forms} = rufus_annotate_globals:forms(Forms),
%%     ExpectedGlobals = #{'Number' => lists:last(Forms)},
%%     ?assertEqual(ExpectedGlobals, Globals).
