...
parse_function_returning_a_cons_expression_test() ->
    ...
    {ok, Tokens} = rufus_tokenize:string(RufusText),
    {ok, Forms} = rufus_parse:parse(Tokens),
    Expected = [{func, #{exprs => [{cons, #{head => {head, #{head => {int_lit, #{line => 1,
                                                                                 spec => 1,
                                                                                 type => {type, #{line => 1,
                                                                                                  source => inferred,
                                                                                                  spec => int}}}},
...
parse_function_that_creates_cons_expression_from_a_variable_and_a_list_literal_test() ->
    ...
    {ok, Tokens} = rufus_tokenize:string(RufusText),
    {ok, Forms} = rufus_parse:parse(Tokens),
    Expected = [{func, #{exprs => [{cons, #{head => {head, #{head => {identifier, #{line => 1,
                                                                                    spec => number}},
                                                             line => 1,
                                                             type => {type, #{line => 1,
                                                                              source => rufus_text,
                                            ...
                                            type => {type, #{collection_type => list,
                                                             ...
                                                             spec => 'list[int]'}}}}],
                         line => 1,
                         params => {params, #{line => 1, params => [{param, #{line => 1,
                                                                              spec => number,
                                                                              type => {type, #{line => 1,
                                                                                               source => rufus_text,
                                                                                               spec => int}}}}]}},
                                   ...
                                   {match, #{left => {identifier, #{line => 4,
                                             ...
                                             right => {list_lit, #{elements => [{int_lit, #{line => 4,
                                                                   ...
                                                                   type => {type, #{collection_type => list,
                                                                                    ...
                                                                                    source => rufus_text,
                                                                                    spec => 'list[int]'}}}}}},
                                   {cons, #{head => {head, #{head => {identifier, #{line => 5,
                                                                                    spec => head}},
                                                             line => 5,
                                                             type => {type, #{line => 5,
                                                                              source => rufus_text,
...
parse_function_taking_a_cons_pattern_test() ->
    ...
    Expected = [{func, #{exprs => [{cons, #{head => {head, #{head => {identifier, #{line => 1,
                                            ...
                                            type => {type, #{collection_type => list,
                                                             element_type => {type, #{line => 1,
                                                                                      source => rufus_text,spec => int}},
                                                             line => 1,
                                                             source => rufus_text,
                                                             spec => 'list[int]'}}}}],
                         line => 1,
                         params => {params, #{line => 1,
                                              params => [{cons, #{head => {head, #{head => {identifier, #{line => 1,
                                                                                                          spec => head}},
                                                                                   line => 1,
                                                                                   type => {type, #{line => 1,
                                                                                                    source => rufus_text,
