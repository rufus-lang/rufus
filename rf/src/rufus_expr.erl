...
typecheck_and_annotate(RufusForms) ->
    ...
    Locals = #{},
    try
        {ok, _Locals, AnnotatedForms} = typecheck_and_annotate(Acc, Stack, Globals, Locals, RufusForms),
...
typecheck_and_annotate_match(Stack, Globals, Locals, Form = {match, Context = #{left := Left}}) ->
    ...
    case rufus_form:has_type(AnnotatedLeft1) of
        true ->
            ok = validate_pattern(Stack, Globals, NewLocals1, AnnotatedForm1),
            typecheck_and_annotate_match_with_bound_left_operand(Stack, Globals, NewLocals1, AnnotatedForm1);
        false ->
            typecheck_and_annotate_match_with_unbound_left_operand(Stack, Globals, NewLocals1, AnnotatedForm1)
    end.
